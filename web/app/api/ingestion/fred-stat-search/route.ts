...
export async function GET(request: Request) {
  ...
  const limit = (() => {
    const raw = searchParams.get("limit") ?? "200";
    const n = Number(raw);
  ...
  try {
    ...
    if (!q) {
      return NextResponse.json({ ok: true, items: [] as unknown[] });
    }

    console.time("fred-stat-search:response");
    const queryText = `
      select
        node_id,
        parent_node_id,
      ...
      where upper(coalesce(node_type, '')) = 'SERIES'
        and upper(coalesce(srch_yn, 'N')) = 'Y'
        and (
          node_name ilike ('%' || $1 || '%')
    ...
    console.time("fred-stat-search:db");
    const result = await client.query(queryText, [q, limit]);
    console.timeEnd("fred-stat-search:db");

    console.time("fred-stat-search:transform");
    ...
    console.timeEnd("fred-stat-search:transform");

    console.timeEnd("fred-stat-search:response");
    return NextResponse.json({ ok: true, items });
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : "FRED 검색 결과를 불러오지 못했습니다.";
