  ...
  try {
    ...
    const result = await client.query(
      `
        select
          ...
          category_sort,
          item_sort
        from dp.api_stat_list_oecd
        where coalesce(srch_yn, 'Y') = 'Y'
        order by category_sort asc nulls last, item_sort asc nulls last, id asc
      `,
    );
    ...
    return NextResponse.json({
      ...
      items: result.rows.map((row) => ({
        ...
        flow_ref: String(row.flow_ref ?? ""),
        data_key: String(row.data_key ?? ""),
        ref_area: String(row.ref_area ?? ""),
        cycle: String(row.cycle ?? "M"),
